...
router.post('/', auth, (req, res) => {
  ...
  if (p.quantity <= 0) return res.status(400).json({ error: 'Producto sin stock disponible' });

  const existing = db.prepare(
    'SELECT * FROM cart_items WHERE user_id = ? AND product_id = ? AND (size IS ? OR size = ?) AND (color IS ? OR color = ?)'
  ).get(req.user.id, product_id, size || null, size || null, color || null, color || null);

  const currentQty = existing ? existing.quantity : 0;
  const newQty = currentQty + Number(quantity);
  if (newQty > p.quantity) {
    return res.status(400).json({ error: `Solo hay ${p.quantity} unidad${p.quantity === 1 ? '' : 'es'} disponible${p.quantity === 1 ? '' : 's'}` });
  }
  ...
  const info = db.prepare('INSERT INTO cart_items (user_id, product_id, quantity, size, color) VALUES (?, ?, ?, ?, ?)').run(req.user.id, product_id, Number(quantity), size || null, color || null);
...
router.put('/:id', auth, (req, res) => {
  ...
  const item = db.prepare(
    'SELECT ci.*, p.quantity as stock FROM cart_items ci JOIN products p ON p.id = ci.product_id WHERE ci.id = ? AND ci.user_id = ?'
  ...
  if (quantity <= 0) {
    ...
    if (quantity > item.stock) {
      return res.status(400).json({ error: `Solo hay ${item.stock} unidad${item.stock === 1 ? '' : 'es'} disponible${item.stock === 1 ? '' : 's'}` });
    }
